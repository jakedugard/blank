...
`npm run release` builds a universal binary, signs it with the Developer ID
certificate in the keychain, and notarizes it through the `blank` notarytool
profile (`xcrun notarytool store-credentials blank …` once per machine).
Then attach `dist/Blank-<version>-universal.dmg` to a GitHub release:

    gh release create v0.2.0 dist/Blank-0.2.0-universal.dmg --title "Blank 0.2.0"
