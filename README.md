...
and it shows it at an exact size with no chrome, ready for CleanShot.
    ...
    npm run build             # dist/Blank-darwin-arm64/Blank.app
    npm run install-app       # build, then replace /Applications/Blank.app

The build is unsigned, which is fine for an app you built on the machine it
runs on. Icons are generated by `build/icons.js`.
